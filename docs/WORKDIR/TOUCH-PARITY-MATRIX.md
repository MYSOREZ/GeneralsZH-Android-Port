...
| **Control group: add a group to the current selection** | Add-team modifier + digit | `MSG_ADD_TEAM<n>`: appends the group without clearing | `SelectionXlat.cpp:1253`+, body `SelectionXlat.cpp:1290`–`1322` | **MISSING** — the group panel only ever sends create or select (`GroupPanel.cpp:150`–`154`) | **DONE 08/09/2026** — a one-shot "+" prefix button on the group row: press it, then press a group, and the panel sends `MSG_META_ADD_TEAM<n>` instead of `MSG_META_SELECT_TEAM<n>`, disarming itself. Not a latching mode, and not a double tap of the group button (the first tap has already recalled, and the engine reads two quick presses as "centre the camera", `SelectionXlat.cpp:1197`) | Medium |
