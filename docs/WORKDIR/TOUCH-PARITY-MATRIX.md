...
| **Control group: add a group to the current selection** | Add-team modifier + digit | `MSG_ADD_TEAM<n>`: appends the group without clearing | `SelectionXlat.cpp:1253`+, body `SelectionXlat.cpp:1290`–`1322` | **MISSING** — the group panel only ever sends create or select (`GroupPanel.cpp:150`–`154`) | Falls out of the "Add" selection toggle proposed above, if the panel consults it when choosing between `MSG_META_ADD_TEAM<n>` and `MSG_META_SELECT_TEAM<n>` | Medium |
