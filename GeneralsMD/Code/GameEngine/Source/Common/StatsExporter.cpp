...
// GeneralsX @feature Android port 23/09/2026 Ported from the GeneralsOnline client
// (StatsExporter.cpp, 1f9491c) so that a replay check on this device writes the same
// event record -- builds with frame and position, kills, captures, energy, and money
// every 30 frames -- that the PC client writes with `-headless -replay <rep> -exportStats`.
// Diffing the two files gives the first frame at which the two simulations disagree
// about something observable, from the PC's own simulation, which the replay's
// one-checksum-per-100-frames cannot.
...
void ExportGameStatsJSON(const AsciiString& replayDir, const AsciiString& replayFileName)
{
	...
	// GeneralsX @feature Android port 23/09/2026 Written uncompressed (.gamestats.json)
	// rather than gzip, and never uploaded: on this port the file only exists to be
	// compared, event by event, with the one the PC client writes for the same replay.
	fprintf(stderr, "[GX-NET] stats: writing %u bytes to %s\n", static_cast<unsigned int>(jsonStr.size()), statsPath.str());
	fflush(stderr);
	FILE *out = fopen(statsPath.str(), "wb");
	if (out != nullptr)
	{
		fwrite(jsonStr.data(), 1, jsonStr.size(), out);
		fclose(out);
	}
	else
	{
		fprintf(stderr, "[GX-NET] stats: failed to open %s for writing\n", statsPath.str());
		fflush(stderr);
	}
