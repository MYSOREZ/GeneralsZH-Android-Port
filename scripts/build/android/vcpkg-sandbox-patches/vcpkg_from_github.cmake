...
")
    ...
    # GeneralsX local build note (23/07/2026): this sandboxed environment's
    # egress policy blocks raw HTTPS to github.com/codeload.github.com
    # (curl -> 403 on every vcpkg_download_distfile of a GitHub archive),
    # but plain `git clone`/`git fetch` against the same host works (routed
    # through a local git proxy already used for every other GitHub access
    # this session makes). Reproduce the archive locally via git instead of
    # the normal hash-checked HTTPS download whenever it's not already
    # present in the downloads cache -- this only changes how the source
    # gets onto disk, not which commit/tag is used, so it's not a way to
    # get untrusted code: REF still pins the exact upstream ref.
    set(archive "${DOWNLOADS}/${downloaded_file_name}")
    if(NOT EXISTS "${archive}")
        set(git_clone_dir "${CURRENT_BUILDTREES_DIR}/src/.gitclone-${sanitized_ref}")
        file(REMOVE_RECURSE "${git_clone_dir}")
        file(MAKE_DIRECTORY "${DOWNLOADS}")
        vcpkg_execute_required_process(
            ALLOW_IN_DOWNLOAD_MODE
            COMMAND git clone --quiet "${github_host}/${org_name}/${repo_name}.git" "${git_clone_dir}"
            WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}"
            LOGNAME "git-clone-${org_name}-${repo_name}"
        )
        vcpkg_execute_required_process(
            ALLOW_IN_DOWNLOAD_MODE
            COMMAND git checkout --quiet "${ref_to_use}"
            WORKING_DIRECTORY "${git_clone_dir}"
            LOGNAME "git-checkout-${org_name}-${repo_name}-${sanitized_ref}"
        )
        vcpkg_execute_required_process(
            ALLOW_IN_DOWNLOAD_MODE
            COMMAND git archive "--format=tar.gz" "--prefix=${repo_name}-${sanitized_ref}/" -o "${archive}" HEAD
            WORKING_DIRECTORY "${git_clone_dir}"
            LOGNAME "git-archive-${org_name}-${repo_name}-${sanitized_ref}"
        )
        file(REMOVE_RECURSE "${git_clone_dir}")
    endif()
